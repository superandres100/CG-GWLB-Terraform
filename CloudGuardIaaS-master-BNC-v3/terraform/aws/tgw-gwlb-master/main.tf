...
resource "aws_route" "subnet_route_entry_publicrt" {
  depends_on = [
    module.launch_vpc.public_rtb, module.tgw-gwlb.tgw_attachment
  ]
  route_table_id            = module.launch_vpc.public_rtb
...
// Public RT re-asociation to Public subneta

resource "aws_route_table_association" "rtb1_to_public_subnet1" {
  route_table_id = module.tgw-gwlb.public_natgw_rtb1
  subnet_id = module.launch_vpc.public_subnets_ids_list[0]
}

resource "aws_route_table_association" "rtb1_to_public_subnet2" {
  route_table_id = module.tgw-gwlb.public_natgw_rtb2
  subnet_id = module.launch_vpc.public_subnets_ids_list[1]
}

resource "aws_route_table_association" "rtb1_to_public_subnet3" {
  count = var.number_of_AZs >= 3 ? 1 :0
  route_table_id = module.tgw-gwlb.public_natgw_rtb3[0]
  subnet_id = module.launch_vpc.public_subnets_ids_list[2]
}

resource "aws_route_table_association" "rtb1_to_public_subnet4" {
  count = var.number_of_AZs >= 4 ? 1 :0
  route_table_id = module.tgw-gwlb.public_natgw_rtb4[0]
  subnet_id = module.launch_vpc.public_subnets_ids_list[3]
}
