...
locals {
  valid_subnet_ids = compact([
    var.transit_gateway_attachment_subnet_1_id,
    var.transit_gateway_attachment_subnet_2_id,
    var.transit_gateway_attachment_subnet_3_id,
    var.transit_gateway_attachment_subnet_4_id
  ])
}

# Create the Transit Gateway Attachment
resource "aws_ec2_transit_gateway_vpc_attachment" "my_attachment" {
  transit_gateway_id = var.tgw_id
  vpc_id             = var.vpc_id
  subnet_ids = local.valid_subnet_ids
  appliance_mode_support = "enable" # Enable appliance mode support
  transit_gateway_default_route_table_association = false
  transit_gateway_default_route_table_propagation = false

   tags = {
    Name = "TGW-Attachment-Security-VPC"
  }
...
# Associate the Transit Gateway attachment with the Transit Gateway route table
resource "aws_ec2_transit_gateway_route_table_association" "transit_gateway_attachment_association" {
  transit_gateway_attachment_id = aws_ec2_transit_gateway_vpc_attachment.my_attachment.id
  transit_gateway_route_table_id  = aws_ec2_transit_gateway_route_table.transit_gateway_route_table.id
}

#Update GWLB and NATGW route tables 

resource "aws_route" "subnet_route_entry_gwlbe1" {
  depends_on = [
    aws_route_table.gwlbe_subnet1_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  route_table_id            = aws_route_table.gwlbe_subnet1_rtb.id
...
resource "aws_route" "subnet_route_entry_gwlbe2" {
  depends_on = [
    aws_route_table.gwlbe_subnet2_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  route_table_id            = aws_route_table.gwlbe_subnet2_rtb.id
  destination_cidr_block    = var.spoke_cidr_block
  gateway_id                = var.tgw_id
...
resource "aws_route" "subnet_route_entry_gwlbe3" {
  depends_on = [
    aws_route_table.gwlbe_subnet3_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 3 ? 1 :0
  route_table_id            = aws_route_table.gwlbe_subnet3_rtb[0].id
  destination_cidr_block    = var.spoke_cidr_block
...
resource "aws_route" "subnet_route_entry_gwlbe4" {
  depends_on = [
    aws_route_table.gwlbe_subnet4_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 4 ? 1 :0
  route_table_id            = aws_route_table.gwlbe_subnet4_rtb[0].id
  destination_cidr_block    = var.spoke_cidr_block
...
resource "aws_route" "subnet_route_entry_natgw1" {
  depends_on = [
    aws_route_table.nat_gw_subnet1_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  route_table_id            = aws_route_table.nat_gw_subnet1_rtb.id
  destination_cidr_block    = var.spoke_cidr_block
  gateway_id                = var.tgw_id
...
resource "aws_route" "subnet_route_entry_natgw2" {
  depends_on = [
    aws_route_table.nat_gw_subnet2_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  route_table_id            = aws_route_table.nat_gw_subnet2_rtb.id
  destination_cidr_block    = var.spoke_cidr_block
  gateway_id                = var.tgw_id
...
resource "aws_route" "subnet_route_entry_natgw3" {
  depends_on = [
    aws_route_table.nat_gw_subnet3_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 3 ? 1 :0
  route_table_id            = aws_route_table.nat_gw_subnet3_rtb[0].id
  destination_cidr_block    = var.spoke_cidr_block
...
resource "aws_route" "subnet_route_entry_natgw4" {
  depends_on = [
    aws_route_table.nat_gw_subnet4_rtb, aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 4 ? 1 :0
  route_table_id            = aws_route_table.nat_gw_subnet3_rtb[0].id
  destination_cidr_block    = var.spoke_cidr_block
...
///// Public Route Tables via NATGW

resource "aws_route_table" "public_subnet_rtb1" {
    depends_on = [
    aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  vpc_id = var.vpc_id
  route{
    cidr_block = "0.0.0.0/0"
...
resource "aws_route_table" "public_subnet_rtb2" {
    depends_on = [
    aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  vpc_id = var.vpc_id
...
resource "aws_route_table" "public_subnet_rtb3" {
    depends_on = [
    aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 3 ? 1 :0
  ...
  tags = {
    Name = "Public Subnet Route Table 3"
  }
}

resource "aws_route_table" "public_subnet_rtb4" {
  depends_on = [
    aws_ec2_transit_gateway_vpc_attachment.my_attachment
  ]
  count = var.number_of_AZs >= 4 ? 1 :0
  ...
  tags = {
    Name = "Public Subnet Route Table 4"
  }
}
